    ...
    , ui(new Ui::MainWindow)
{
    ...
    MainWindow::setGeometry(0, 0, 1200, 600);

    translator = new QTranslator;

    fileBrowser = new FileBrowser();
    fileBrowser->setFixedWidth(1200);

    fileBrowser->moveToThread(thread);

    mdiArea = new QMdiArea(this);
    QWidget *wgt = new QWidget(this);
    setCentralWidget(wgt);
    wgt->setFixedWidth(1200);
    QHBoxLayout *layoutMain = new QHBoxLayout(this);
    QGridLayout *layout = new QGridLayout(this);
    wgt->setLayout(layoutMain);
    //wgt->setGeometry(500, 60, 271, 511);

    QWidget *wgtSub = new QWidget(this);
    wgtSub->setLayout(layout);

    layoutMain->addWidget(wgtSub);
    layoutMain->addWidget(fileBrowser);
    ...
    QPushButton* addDateTimeButton = new QPushButton(tr("Добавить дату и время"), this);
    connect(addDateTimeButton, SIGNAL(clicked()), this, SLOT(addDateTime()) );

    buttonLayout->addWidget(addDateTimeButton, 2, 0, 1, 1);

    layout->addWidget(buttonBlock, 11, 0 , 2, 1);

    QTextEdit* TextEdit1 = new QTextEdit(this);
    mdiArea->addSubWindow(TextEdit1);
    //mdiArea->addSubWindow(fileBrowser);
...
void MainWindow::addDateTime()
{
    QTextEdit* te = (QTextEdit*)mdiArea->currentSubWindow()->widget();
    today = QDateTime::currentDateTime();
    te->insertPlainText(today.toString("h:m:s ap ddd MMMM d yy"));
}
